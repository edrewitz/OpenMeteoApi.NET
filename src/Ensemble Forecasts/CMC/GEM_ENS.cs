...
using OpenMeteoApiNet.DataFrames.HourlyData.GEM_ENS;
using OpenMeteoApiNet.Variables.HourlyData.GEM_ENS;
...
{
    public static class gemENSHourlyForecastApi
    {
...

        {
            ...
            string url = $"https://ensemble-api.open-meteo.com/v1/ensemble?latitude={latitude}&longitude={longitude}" +
                $"&hourly={modelParams}" +
                $"&models=gem_global_ensemble&forecast_days={days}" +
            ...
            // Deserialize the "hourly" property into our gemENSParams class. If deserialization fails, print an error message and return.
            var data = JsonSerializer.Deserialize<gemENSParams>(hourlyWeatherElement.GetRawText());
            ...
            if (data != null)
            {
                ...
                var df = gemENSDataFrame.ToDataFrame(data);
                ...
                if (toCsv == true)
                {
                    ...
                    if (fileName == null)
                    {
                        ...
                        fileName = $"GEM_ENS_PointForecast_{latString}_{lonString}.csv";
                    }
                    else
                    {
                        fileName = fileName;
            ...
            {
                Console.WriteLine($"GEM ENS Data Not Available At This Time");
                return null;
