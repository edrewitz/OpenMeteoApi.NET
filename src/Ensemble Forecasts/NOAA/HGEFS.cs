...
using OpenMeteoApiNet.Variables.HourlyData.HGEFS;
...
namespace OpenMeteoApiNet.EnsembleForecasts.NOAA.HGEFS
{
    public static class hgefsHourlyForecastApi
    {
        ...
        private static DataFrame ToDataFrame(hgefsParams data)
        {
             ...
             * -------
             * 
             * Microsoft.Data.Analysis.DataFrame object containing the data from the hgefsParams object.
             * 
             */
...

        {
            ...
            if (days > 10)
            {
                Console.WriteLine("The 'days' parameter cannot exceed 10. Setting 'days' to 10.");
                days = 10;
            }
            ...
            string url = $"https://api.open-meteo.com/v1/forecast?latitude={latitude}&longitude={longitude}" +
                $"&hourly={modelParams}" +
                $"&models=ncep_hgefs025_ensemble_mean&forecast_days={days}" +
            ...
            if (data != null)
            {
                ...
                if (toCsv == true)
                {
                    ...
                    if (fileName == null)
                    {
                        ...
                        fileName = $"HGEFS_PointForecast_{latString}_{lonString}.csv";
                    }
                    else
                    {
                        fileName = fileName;
